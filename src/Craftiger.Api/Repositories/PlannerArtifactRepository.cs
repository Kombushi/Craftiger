    ...
    ILogger<PlannerArtifactRepository> logger) : IPlannerArtifactRepository
{
    ...
    public PlannerArtifact Load(string artifactsDir)
    {
        ...
        var items = db.Query<ItemRow>(
                "SELECT id, name_en AS Name, is_fluid AS IsFluid, leaf_class AS LeafClass, atlas_idx AS AtlasIdx, max_stack AS MaxStack FROM items")
            .ToDictionary(
                row => row.Id,
                row => new ArtifactItem(
                    row.Id, row.Name, row.IsFluid != 0, row.LeafClass, row.AtlasIdx, (int?)row.MaxStack,
