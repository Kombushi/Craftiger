...
public class FactoryEndToEndTests
{
    ...
    {
        // The acceptance check: a boiler line boils fuel and water into steam, a steam
        // machine drinks it in place of EU, and the carrier balances as ordinary item flow.
        var graph = SolverGraph.Build(
        ...
        var machines = new FactoryMachineData(
            new Dictionary<string, IReadOnlyList<FactoryMachineBlock>>
            {
                ...
                ["Mill"] =
                [
                    // A GT++-shape steam multi: eight parallels at 125 % speed and 62.5 %
                    // steam use — the machines layer is where steam legitimately wins.
    ...
    public void SingleSteamTurbineConvertsAtItsEfficiency()
    {
        ...
        var machines = new FactoryMachineData(
            new Dictionary<string, IReadOnlyList<FactoryMachineBlock>>
            {
                ["Steam Turbine"] =
                [
                    new FactoryMachineBlock(
                        "basic-st", 1, false, false, 0, 1, [], 85.714285714285708, 32, 1),
                ],
            },
            [],
        ...
        var plan = Solve(
            graph,
            new FactoryRequest(
                [new FactoryTarget(FactoryTargetKind.Energy, null, 31)],
                [], new Dictionary<string, string>()),
            machines: machines);
    ...
    public void LargeSteamTurbineReturnsDistilledWater()
    {
        ...
        var machines = new FactoryMachineData(
            new Dictionary<string, IReadOnlyList<FactoryMachineBlock>>
            {
                ["Large Steam Turbine"] =
                [
                    new FactoryMachineBlock("lst", null, true, false, 0, 1, [], RotorTurbine: true),
                ],
            },
            [],
        ...
        Assert.Equal(FactoryPlanStatus.Solved, plan.Status);
        // 3,600 EU/t of flow at 0.5 EU/L: 144,000 L/s of steam, one liter of distilled
        // water back per 160.
        var line = plan.Lines.MaxBy(l => l.RunsPerSecond)!;
